...
- **Read — session forensics (no consent needed):** `list_recent_sessions`, `search_sessions`, `get_session_summary`, `get_session_console_errors`, `get_session_network_errors`, `get_user_action_before_error`, `get_dom_snapshot`, `query_dom_history`, `generate_playwright_repro`.
...
- **Audit — integrity check (read-only, no consent):** `verify_audit_log`.
