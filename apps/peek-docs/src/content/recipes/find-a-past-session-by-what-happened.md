...
Returns an array of matching sessions — same shape as `list_recent_sessions` — each with `id`, `origin`, `startedAt`, `durationMs`, and `eventCount`.
...
search_sessions({
  ...
  createdAfter: "2026-06-24",
  createdBefore: "2026-06-25",
  hasNetworkErrors: true
})
```

`hasConsoleErrors` and `hasNetworkErrors` are booleans that filter to only sessions where peek captured at least one error of that type; combine them for sessions that have both.
...
| `--status <value>` | Filter by status (`active`, `finalized`) |
| `--errors <type>` | Only sessions with errors: `console`, `network`, or `any` |
| `--limit <n>` | Max rows to return (default: 20) |
| `--json` | Emit rows as a pretty-printed JSON array for scripting |

**`--json` example** (pipe to `jq` for scripting):

```bash
peek sessions search --q "checkout" --json | jq '.[0].id'
```
