...
tracelane's artifact is per-failure, not a single aggregate report, and filenames carry a per-project id + a millisecond timestamp — so shards and parallel workers stay distinct and **no merge step is required**. `download-artifact`'s `merge-multiple` just flattens the per-shard folders into one.
