...
| A.3 | Version reporting | `npx -y @peekdev/mcp --version` — **NOTE: peek-mcp does NOT implement `--version`.** Expected to ignore the flag and start the server. The bin's version lives in `package.json` (`0.1.0-alpha.2`). Status this as ⏸ if the flag does nothing — that's the actual behavior. If we want a `--version` flag, that's a 🟢. | |
