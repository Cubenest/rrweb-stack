...
|---|---|---|---|
| A.1 | Server starts on `npx` and answers `initialize` | `npx -y @peekdev/mcp@latest` in a fresh shell. Process starts, waits on stdin. In another shell pipe: `printf '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"qa","version":"0.0.0"}}}\n' \| npx -y @peekdev/mcp@latest` (or use Inspector — `npx -y @modelcontextprotocol/inspector npx -y @peekdev/mcp@latest`). **Expected:** a JSON-RPC response with `serverInfo.name = "peek-mcp"` and `serverInfo.version` matching the installed package's `package.json` version (currently `0.1.0-alpha.2`). | |
...
| A.3 | Version reporting | `npx -y @peekdev/mcp@latest --version` — **NOTE: peek-mcp does NOT implement `--version`.** Expected to ignore the flag and start the server. The bin's version lives in `package.json` (`0.1.0-alpha.2`). Status this as ⏸ if the flag does nothing — that's the actual behavior. If we want a `--version` flag, that's a 🟢. | |
...
| A.5 | Manifest landing location after `peek init` | After running Group B.3 below: `ls ~/Library/Application\ Support/Google/Chrome/NativeMessagingHosts/`. **Expected:** `com.cubenest.peek.json` exists. (Linux: `~/.config/google-chrome/NativeMessagingHosts/`. Windows: `HKCU\Software\Google\Chrome\NativeMessagingHosts\com.cubenest.peek` registry key.) Note: `com.cubenest.peek` — the prefix is the GitHub org / `@cubenest/rrweb-core` scope, not the `@peekdev` npm scope. | |
| A.6 | Manifest content sanity | `cat ~/Library/Application\ Support/Google/Chrome/NativeMessagingHosts/com.cubenest.peek.json`. **Expected:** `name = "com.cubenest.peek"`, `type = "stdio"`, `allowed_origins` includes the dev-build extension ID(s) from `packages/peek-mcp/src/native-host/extension-ids.json`, `path` points at a `peek-mcp` shim. | |
