...
| C.4 | Replay actually works | Click play / drag the scrubber. **Expected:** DOM evolves — the input typed, the button clicked. Open the console panel → see the deliberate `console.log("[tracelane-demo] button clicked")`. Open the network panel → see the failed `/api/will-fail` request. | |
