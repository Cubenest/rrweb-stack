...
- `pnpm lint` / `pnpm lint:fix` — Biome check (and autofix) the repo. Note
  Biome ignores `apps/**`, `.astro`, and build output.
- `pnpm changeset` — author a changeset for any user-visible change to a
  published package (required; see conventions below).

Scope work to one package with a filter, e.g.
`pnpm --filter @peekdev/extension build` or
`pnpm --filter @tracelane/wdio test`. The Chrome MV3 extension is the WXT app
at `packages/peek-extension/`; the two Astro docs sites live in `apps/`.
