...
export const SERVER_INSTRUCTIONS =
  ...
  "not take, stop and report via set_intent with a short message and status:'failed' rather than retrying blindly; when the whole task is complete, call set_intent with a summary and status:'done'. " +
...
export function createPeekMcpServer(options: CreatePeekMcpServerOptions = {}): PeekMcpServer {
  ...
  function registerTools(): void {
    ...
    // 18. request_pairing (SP4) -----------------------------------------------
    // A connector presents its pairing code; the server derives the client name
    // from the MCP connection's clientInfo (NOT a user-supplied arg — a connector
    // cannot spoof it). The bridge relays the pair.request round-trip to the SW,
    // which matches the code and (on approval) returns a one-shot secret. Every
    // attempt — approved or denied — is audit-logged; the secret is NEVER written.
    server.registerTool(
      'request_pairing',
      {
        title: 'Request connector pairing',
        description:
        ...
        inputSchema: {
          code: z
            .string()
            .describe('The pairing code generated by the connector for this handshake.'),
        },
        annotations: {
          readOnlyHint: false,
          destructiveHint: false,
  ...
  // --- Pairing dispatch (SP4 request_pairing) ---
  async function dispatchPairingRequest(input: {
    code: string;
  }): Promise<ReturnType<typeof jsonResult>> {
    const bridge = options.hostBridge ?? new MissingHostBridge();
    const clientImpl = server.server.getClientVersion();
    ...
    const auditResult: AuditResult = response.approved ? 'ok' : 'denied';
    const draft: DraftAuditEntry = {
      ts: new Date(requestStartedAtMs).toISOString(),
      tool: 'request_pairing' as AuditTool,
      // args captures the code only — never the returned secret.
    ...
    try {
      ...
      console.error(
        `peek-mcp: pairing audit log write failed — ${err instanceof Error ? err.message : String(err)}`,
      );
    }

    // Return the result to the connector. The secret is included only on approval.
    return jsonResult({
