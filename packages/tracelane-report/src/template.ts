...
/* Custom timeline strip under the player. The rrweb-player has its own scrubber;
   this strip shows the OVERALL session timeline with the failure marker. */
.timeline {
  position: relative;
  display: flex;
  flex-direction: column;
...
.timeline-track .fill {
  ...
  background: linear-gradient(90deg, transparent, rgba(94, 234, 212, 0.06) 50%, transparent);
}
.timeline-track .failure-marker {
...
@keyframes marker-pulse {
  0%, 100% { opacity: 1; box-shadow: 0 0 18px var(--amber); }
  50%      { opacity: 0.6; box-shadow: 0 0 10px var(--amber); }
}
...
/** Render the custom timeline strip (under the rrweb-player). */
function renderTimeline(firstTs: number, lastTs: number, status: string): string {
  if (firstTs === 0 || lastTs === 0 || lastTs <= firstTs) {
    // Not enough recording to plot a timeline.
    return '';
  }
  const totalMs = lastTs - firstTs;
  const isFailure = status === 'failed' || status === 'broken';
  const markerLeft = '100%'; // failure-as-end-of-recording approximation
  const labelHtml = isFailure
    ? `<div class="failure-label" style="left:${markerLeft};transform:translateX(-100%)">${escapeHtml(status)}</div>`
    : '';
  const markerHtml = isFailure
    ? `<div class="failure-marker" style="left:${markerLeft}"></div>`
    : '';
  return `
<div class="timeline">
  ...
  <div class="timeline-axis">
    <span>+0:00</span>
    <span>+${formatRelativeMs(totalMs / 4)}</span>
    <span>+${formatRelativeMs(totalMs / 2)}</span>
    <span>+${formatRelativeMs((totalMs * 3) / 4)}</span>
    <span>+${formatRelativeMs(totalMs)}</span>
  </div>
</div>`;
}
...
<main class="investigation">
  <section class="replay" aria-label="Session replay">
    ...
    ${renderTimeline(firstTs, lastTs, meta.status)}
  </section>
  <aside class="panels" aria-label="Investigation panels">
    ...
    <div class="panel-pane" id="pane-timeline" role="tabpanel">
      <div class="coming-soon">
        ...
        <span>The overview strip under the replay shows session bounds + the failure marker today.</span>
