...
import {
  ACTION_FEEDBACK_HOST_ATTR,
  RECORDER_BLOCK_SELECTOR,
...
// receives — they fail if any overlay marker is dropped from the wiring.
// (The closed-shadow-sweep skip is covered behaviorally in shadow.test.ts.)
describe('recorder blockSelector', () => {
  it('blocks the recording-frame, shield, and action-feedback hosts', () => {
    ...
    expect(RECORDER_BLOCK_SELECTOR).toContain(`[${ACTION_FEEDBACK_HOST_ATTR}]`);
  });
  ...
  it('uses distinct markers for the three overlays', () => {
    expect(SHIELD_HOST_ATTR).toBe('data-peek-shield-host');
    expect(SHIELD_HOST_ATTR).not.toBe(RECORDING_FRAME_HOST_ATTR);
    expect(ACTION_FEEDBACK_HOST_ATTR).not.toBe(SHIELD_HOST_ATTR);
    expect(ACTION_FEEDBACK_HOST_ATTR).not.toBe(RECORDING_FRAME_HOST_ATTR);
  });
});
